[22:30, 04/01/2026] Simon MOISSELIN: # 
...
*worker* : Service .NET qui récupère les votes depuis Redis et les enregistr…
[22:31, 04/01/2026] Simon MOISSELIN: # 
...
Docker Swarm permet de déployer l'application sur plusieurs machines. J'ai configuré un cluster avec 1 nœud manager et 2 nœuds workers.
...
Il faut installer docker sur chaque machine.
...
*Sur le manager* :
bash
hostname -I # me donne l'adresse IP de la machine manager
docker swarm init --advertise-addr <IP_MANAGER>
docker swarm join-token worker ## cette commande permet de récupérer le token pour rejoindre le swarm


*Sur chaque worker en utilisant le token* :
bash
...
Cette commande permet de lister les services en cours d'exécution sur le cluster, et de debugger les services en cas de problème.

On peut tester de supprimer un nœud worker et vérifier que les services continuent de fonctionner. c'est le cas normalement, car les replicas de vote et result permettent de maintenir la disponibilité de l'application.
